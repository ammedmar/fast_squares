...
%Let $X$ be a simplicial complex and $\alpha$ a cochain whose support is the subset $A = \{a_1, \dots, a_m\}$ of $X_n$.
%Denote by $\alpha_i$ the cochain dual of $a_i$ and consider $k \in \{1, \dots, n\}$.
...
%\end{equation*}
%where $\ind(S) = \{\ind(v) \mid v \in S\}$ for any $S \subseteq \overline{a}_{ij}$ and $S \xor S^\prime = S \cup S^\prime \setminus (S \cap S^\prime)$.
%
%\anibal{improve this last paragraph}
%There
